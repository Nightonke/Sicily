...
void solve() {
    ...
    for (int i = 0; i < N; i++) G[i][i] |= 1;  // 仅将最低位设为一，不能影响次低位
    for (int k = 0; k < N; k++) {
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                if (G[i][k] && G[k][j]) G[i][j] |= 1;  // i <(<=) k && k <(<=) j我们都可以认为i <= j
            }
        }
    }
    for (int i = 0; i < N && ok; i++) {
        for (int j = 0; j < N; j++) {
            if ((G[i][j] & 2) && G[j][i]) {  // 注意只有11和10与2才为真，也即次低位要是1才行，也即i<j,而G[j][i]11、10、01都为真
                ok = false;
                break;
            }
        }
    ...
    int s = unEqual.size();
    for (int i = 0; i < s; i++) {
        if ((G[unEqual[i].first][unEqual[i].second] & 1) && (G[unEqual[i].second][unEqual[i].first] & 1)) {  // 注意只有11和01，也即最低位是1与1才为真，这里保证了first<=second且second<=first
            ok = false;
            break;
        }
    }
